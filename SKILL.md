...
**阶段一 · 建模**——用户给了原始素材（聊天记录、邮件、访谈、文档），目标是产出带生成时间的 PSP 主文档和 current 入口。**关键词**：建模某人、整理某人的特征、提炼经验模式、构建 PSP。

**阶段二 · 扮演**——用户已有 `PSP-YYYYMMDD-HHMMSS.md` 或 `PSP.md` current 入口，目标是生成带生成时间的可运行 system prompt。**关键词**：生成 prompt、做成 Claude 项目、让 AI 扮演、output system prompt。
...
people/
  └── {person_id}/                    ← 用人名拼音或代号，避免空格
      ├── PSP-YYYYMMDD-HHMMSS.md      ← 阶段一版本产物，PSP 主文档
      ├── PSP.md                      ← current 入口，指向/同步最新版本
...
此时 canonical PSP 产物会写入 `identity/psp/<person_id>/PSP-YYYYMMDD-HHMMSS.md` 和 `identity/psp/<person_id>/PSP.md`。原始素材默认不进入公开 LifeOS repo；应留在授权私有资料源，或临时放在 PSP repo 中已被 gitignore 的 `people/{person_id}/raw_materials/` 工作区。

**新建人物目录时**：默认运行 `scripts/init_person.sh <person_id>`，产出到 `people/<person_id>/`；如需指定目录，用 `--output-root <dir>`；如需 openLifeOS 产物，用 `--lifeos-root <lifeos_repo>`。
...
默认：`people/{person_id}/PSP-YYYYMMDD-HHMMSS.md`——按 v2.1 完整结构填充的版本产物。

默认：`people/{person_id}/PSP.md`——current 入口，供 agent/runtime 默认读取。

LifeOS 模式：`identity/psp/{person_id}/PSP-YYYYMMDD-HHMMSS.md` + `identity/psp/{person_id}/PSP.md`。
...
默认产出物写入 `people/{person_id}/system_prompt-YYYYMMDD-HHMMSS.txt`。LifeOS 模式写入 `identity/psp/{person_id}/system_prompt-YYYYMMDD-HHMMSS.txt`。建议附一段试运行说明：用 5-10 个典型问题在 Claude 项目里跑一遍，肉眼检查是否明显失真，再进入阶段三的自动化测试。
